...
function main()
    ...
    % 读取加密图像
    enc_img_path = "./图片/彩色叠加/woman.jpeg";
    ...
    imshow(enc_img);
    title("加密图像");
    imwrite(enc_img, './图片/彩色叠加/加密图像.jpeg');% 保存加密图像副本

    % 读取原始图像1（表情符号）
    original_img1_path = "./图片/彩色叠加/emoji.jpeg";
    ...
    title("原始图像1");
    imwrite(original_img1, './图片/彩色叠加/原始图像1.jpeg');  % 保存原始图像1副本

    % 读取原始图像2（鸟类）
    original_img2_path = "./图片/彩色叠加/bird.jpeg";
    ...
    title("原始图像2");
    imwrite(original_img2, './图片/彩色叠加/原始图像2.jpeg');  % 保存原始图像2副本
    ...
    % 解析图像尺寸参数
    x = enc_img_size(1);  % 图像高度
    y = enc_img_size(2);  % 图像宽度
    z = enc_img_size(3);  % 颜色通道数（RGB=3）
    disp("图像高度: "); disp(x);
    disp("图像宽度: "); disp(y);
    disp("颜色通道数: "); disp(z);
 
    % 初始化全白矩阵（用于半色调处理）
    halftone_enc_img = 255 * ones(enc_img_size);        % 加密图像半色调矩阵
    halftone_original_img1 = 255 * ones(enc_img_size);  % 原始图像1半色调矩阵
    halftone_original_img2 = 255 * ones(enc_img_size);  % 原始图像2半色调矩阵
    
    % 初始化双倍尺寸矩阵（用于图像分割）
    img1 = 255 * ones(2*x, 2*y, z);  % 分割图像1矩阵
    img2 = 255 * ones(2*x, 2*y, z);  % 分割图像2矩阵
    
    % 初始化合并结果矩阵
    merged_img = 255 * ones(2*x, 2*y, z);  % 最终合并图像矩阵
 
    % 多通道处理循环
    for i = 1 : 3  % 遍历RGB三个颜色通道
        % 半色调处理（二值化）
        ...
        % 图像分割（将两个原始图像嵌入到加密图像中）
        [img1(:, :, i), img2(:, :, i)] = img_divide(...
            halftone_enc_img(:, :, i), ...
            halftone_original_img1(:, :, i), ...
            halftone_original_img2(:, :, i));
    ...
    % 保存中间处理结果（中文名称）
    imwrite(halftone_enc_img, './图片/彩色叠加/半色调加密图像.bmp');
    imwrite(halftone_original_img1, './图片/彩色叠加/半色调原始图像1.bmp');
    imwrite(halftone_original_img2, './图片/彩色叠加/半色调原始图像2.bmp');
    imwrite(img1, './图片/彩色叠加/分割图像1.bmp');
    imwrite(img2, './图片/彩色叠加/分割图像2.bmp');
    imwrite(merged_img, './图片/彩色叠加/最终合并图像.bmp');
 
    % 显示处理结果
    figure; imshow(halftone_enc_img); title("半色调加密图像");
    figure; imshow(halftone_original_img1); title("半色调原始图像1");
    figure; imshow(halftone_original_img2); title("半色调原始图像2");
    figure; imshow(img1); title("分割图像1");
    figure; imshow(img2); title("分割图像2");
    figure; imshow(merged_img); title("最终合并图像");
end
