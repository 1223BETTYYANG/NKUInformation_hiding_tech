% Pre
img = (imread('lena.bmp'));
watermark_gray = rgb2gray(imread('NK.jpg'));
watermark = imbinarize(watermark_gray);
...
subplot(233); imshow(imcomplement(watermark));title("反色前水印");
subplot(234); imshow(new_image,[]);title("嵌入水印");
subplot(235); imshow(vec,[]);title("提取图像");
subplot(236); imshow(imcomplement(vec),[]);title("提取图像后反色comp原图");